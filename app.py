        ...
        try:
            if "picture" in request.files:
                ...
                if emotion:
                    ...
                    track_html = populate_tracks(tracks)
                else:
                    emotion_html = '<h3 style="text-align: center;">No face detected</h3>'
                    track_html = ''  # Provide empty tracks HTML

                return jsonify(emotion_html=emotion_html, track_html=track_html)
    ...
    if request.method == "POST":
        try:
            ...
            if song_file and allowed_file(song_file.filename):
                ...
                new_song = Song(title=title, singer=singer, emotion=emotion, song_path=song_path, rating=rating)
...
def songs():
    try:
        songs = Song.query.filter(Song.emotion == 'happy').all()
